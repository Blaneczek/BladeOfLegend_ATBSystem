...
FCombatActionData ABLCombatEnemy::GetEnemyAction()
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("BLCombatEnemy | AvailableActions is empty"));
		return FCombatActionData();
	}

	const int32 RandomIndex = FMath::RandRange(0, AvailableActions.Num() - 1);
	if (UBLAction* Action = Cast<UBLAction>(AttackActions[AvailableActions[RandomIndex]].LoadSynchronous()->GetDefaultObject()))
	{
		if (Action->TurnsCost > 0)
		{
			ActionsCooldown.Add(AvailableActions[RandomIndex], Action->TurnsCost);
		}
		return FCombatActionData(ECombatActionType::ATTACK, Action->Flow, AvailableActions[RandomIndex],
								ECrystalColor::NONE, 0.f, Action->TargetsNumber);
	}
