...
{
	return GetCharacter() ? GetCharacter()->GetCurrentCooldown() : 0.f;
}
...
{
	if (IsValid(Character) && GetWorld()->GetTimerManager().IsTimerActive(Character->GetCooldownTimer()))
	{
...
{
	if (IsValid(Character) && GetWorld()->GetTimerManager().IsTimerPaused(Character->GetCooldownTimer()))
	{
...
{
	if (bClicked || !IsValid(TargetPointer))
	{
		return;
	}
