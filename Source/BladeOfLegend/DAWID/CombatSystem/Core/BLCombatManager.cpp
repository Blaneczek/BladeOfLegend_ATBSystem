...
void ABLCombatManager::SetHeroesOverworldHP(float MinPercentage)
{
	...
	for (int32 Index = 0; Index < GI->SaveGameData.HeroesData.Heroes.Num(); ++Index)
	{
		...
		{
			GI->SaveGameData.HeroesData.Heroes[Index].HeroAttributes.CurrentHP = PlayerTeam[Index]->GetCharacter()->GetCurrentHP()
																				> PlayerTeam[Index]->GetCharacter()->GetMaxHP() * MinPercentage
																				? PlayerTeam[Index]->GetCharacter()->GetCurrentHP()
																				: PlayerTeam[Index]->GetCharacter()->GetMaxHP() * MinPercentage;
		}
	}
}
...
void ABLCombatManager::HeroDied(ABLCombatSlot* PlayerSlot)
{
	if (IsValid(Widget))
	{
		PlayerSlot == CurrentPlayerSlot ? Widget->HeroDied(PlayerSlot->GetIndex(), true)
										: Widget->HeroDied(PlayerSlot->GetIndex(), false);
	}
...
void ABLCombatManager::SetEnemyTeam()
{
	...
	{
		if (EnemyTeam[Index] && IsValid(Widget))
		{
...
{
	if (ABLCombatPlayerController* AC = Cast<ABLCombatPlayerController>(UGameplayStatics::GetPlayerController(GetWorld(), 0)))
	{
...
void ABLCombatManager::BindSlotsDelegetes()
{
	for (const auto& Slot : PlayerTeam)
	{
		if (Slot && Slot->IsActive())
		{
			...
			Slot->OnSelectedSlot.BindWeakLambda(this, [this, Slot](ABLCombatSlot* InSlot)
				{
					if (!IsValid(CurrentPlayerSlot)) ChoosePlayerSlot(Slot);
				});
			Slot->OnCharHealthUpdate.BindWeakLambda(this, [this, Slot]()
				{
					if (IsValid(Widget)) Widget->UpdateHeroHealth(Slot->GetIndex(), Slot->GetCharacter()->GetMaxHP(), Slot->GetCharacter()->GetCurrentHP());
				});
	...
	for (const auto& Slot : EnemyTeam)
	{
		if (Slot)
		{
			...
			if (IsValid(Widget))
			{
				Slot->OnCharHealthUpdate.BindWeakLambda(this, [this, Slot]()
					{
						Widget->UpdateEnemyHealth(Slot->GetIndex(), Slot->GetCharacter()->GetCurrentHP(), Slot->GetCharacter()->GetMaxHP());
					});
				Slot->OnCharSpawned.BindWeakLambda(this, [this](int32 Index, const FString& Name, int32 Level, float Cooldown)
					{
						Widget->AddEnemy(Index, Name, Level, Cooldown);
					});
				Slot->OnCharDestroyed.BindWeakLambda(this, [this](int32 Index)
					{
						Widget->RemoveEnemy(Index);
					});
			}
		}
	}
